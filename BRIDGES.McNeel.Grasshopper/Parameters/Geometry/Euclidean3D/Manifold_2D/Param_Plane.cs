...
namespace BRIDGES.McNeel.Grasshopper.Parameters.Geometry.Euclidean3D
{
    ...
    public class Param_Plane : GH_Kernel.GH_Param<Gh_Plane>, GH_Kernel.IGH_PreviewObject
    {
        ...
        /// </summary>
        public Param_Plane()
          : base("Plane", "Plane", "Contains a collection of planes in a three-dimensional euclidean space.", "BRIDGES Basics", "Parameters", GH_Kernel.GH_ParamAccess.item)
        {
            this.Hidden = false;
        }
            ...
            if (dataCount != 0)
            {
                for (int i_B = 0; i_B < m_data.Branches.Count; i_B++)
                {
                    ...
                    for (int i_I = 0; i_I < branch.Count; i_I++)
                    {
                        ...
                        if (item != null)
                        {
                            Draw.Plane(args.Display, item.Value, isSelected);
                        }
                    }
                }
            }
